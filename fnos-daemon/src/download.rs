//! frpc 二进制下载：平台匹配 / 流式下载 / Range 续传 / sha256 校验。
//!
//! 规格源：src-tauri/src/commands/download.rs（常量与逻辑照抄，
//! 数据目录与进度事件替换为 daemon 自有依赖）。
...
async fn get_download_info() -> Result<DownloadInfo, String> {
    ...
    let info_response: serde_json::Value = response
        .json()
        .await
    ...
    let downloads = info_response["data"]["downloads"]
        .as_array()
        .cloned()
        .unwrap_or_default();
    ...
    let mut matched: Vec<&serde_json::Value> = downloads
        .iter()
        .filter(|d| d["platform"] == serde_json::json!(platform))
        .collect();
    ...
    if matched.is_empty() {
        ...
        matched = downloads
            .iter()
            .filter(|d| {
                d["os"] == serde_json::json!(target_os)
                    && matches_arch(
                        os,
                        arch,
                        d["arch"].as_str().unwrap_or_default(),
                    )
            })
            .collect();
    }

    let download = match matched.len() {
        0 => return Err(format!("No matching download found for platform: {} {}", os, arch)),
        1 => matched[0],
        _ => matched.iter().max_by_key(|d| d["size"].as_u64().unwrap_or(0)).unwrap(),
    };

    Ok(DownloadInfo {
        url: download["link"].as_str().unwrap_or_default().to_string(),
        hash: download["hash"].as_str().unwrap_or_default().to_string(),
        size: download["size"].as_u64().unwrap_or(0),
    })
}

/// 下载 frpc 到数据目录（流式 + Range 续传 + sha256 校验 + 进度事件）。
pub async fn download_frpc(frpc: &Arc<FrpcManager>) -> Result<String, String> {
    ...
    let frpc_path = frpc.data_dir.join(if cfg!(target_os = "windows") {
        "frpc.exe"
    } else {
        "frpc"
    });
    std::fs::create_dir_all(frpc.data_dir.clone()).map_err(|e| e.to_string())?;
    ...
    let bypass_proxy = std::env::var("BYPASS_PROXY")
        ...
        .parse::<bool>()
        .unwrap_or(true);
    let client = if bypass_proxy { client.no_proxy() } else { client }
        .build()
    ...
    loop {
        ...
        if status.as_u16() == 206 {
            if let Some(range_str) = response.headers().get("content-range").and_then(|v| v.to_str().ok()) {
            ...
            match item {
                Ok(chunk) => {
                    ...
                    if this_chunk_size >= PROGRESS_EMIT_THRESHOLD {
                        let _ = frpc.events.send(Event::download_progress(DownloadProgress {
                            downloaded,
                            total: total_size,
                            percentage,
                        }));
...
/// 查询 frpc 是否已就绪（D2：内置/已下载优先）。
pub fn check_frpc_exists(frpc: &Arc<FrpcManager>) -> bool {
    frpc.resolve_frpc_path().map(|p| p.exists()).unwrap_or(false)
}

pub fn get_frpc_directory(frpc: &Arc<FrpcManager>) -> String {
    frpc.data_dir.to_string_lossy().to_string()
}
